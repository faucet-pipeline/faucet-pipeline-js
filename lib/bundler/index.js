...
module.exports = class Bundler {
	constructor(configs, referenceDir, { compact }, handler) {
		this.configs = configs;
		this.refDir = referenceDir;
	...
	generateAll() {
		this.configs.forEach(config => {
			...
			let entryPointPath = path.resolve(this.refDir, entryPoint);
			...
			this._bundles[entryPoint] = {
				...
				files: [fs.realpathSync(entryPointPath)]
			};
...
function collectModulePaths(memo, module) {
	...
	try {
		// XXX: is there a risk here WRT relative paths?
