...
module.exports = (config, { rootDir, configDir }, { watcher, fingerprint, compact }) => {
	...
	mkdirp(targetDir, err => {
		...
		start(config.bundles, targetDir, // eslint-disable-next-line indent
				config.manifest, { watcher, fingerprint, compact });
	});
};

function start(bundleConfigs, targetDir, manifest, { watcher, fingerprint, compact }) {
	let onBundle = (entryPoint, code, error) => {
		let terminate = error && !watcher && (_ => process.exit(1));

		let res = writeBundle(entryPoint, // eslint-disable-next-line indent
				targetDir, code, manifest, { fingerprint }).
			catch(err => {
	...
	let bundler = new Bundler(bundleConfigs, { compact }, onBundle);
	if(watcher) {
		watcher.on("edit", bundler.rebuild.bind(bundler));
	}
}

function writeBundle(entryPoint, targetDir, code, manifest, { fingerprint }) {
	...
	if(fingerprint) {
		filename = generateFingerprint(filename, code);
	}

	let res = new SimplePromise();
	let filepath = path.resolve(targetDir, path.basename(filename));
