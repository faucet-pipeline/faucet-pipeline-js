...
module.exports = (config, assetManager, { browsers, compact, sourcemaps } = {}) => {
	...
	return filepaths => Promise.all(bundles.map(bundle => {
		let options = bundle.fingerprint === undefined ? {} :
				{ fingerprint: bundle.fingerprint }; // eslint-disable-line indent
		let writer = makeWriter(bundle, assetManager, options);
