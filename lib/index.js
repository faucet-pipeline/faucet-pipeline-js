...
function generateManifest(entryPoint, bundlePath, { file, baseURI = "" }) {
	MANIFEST[entryPoint] = baseURI.call ?
			baseURI(bundlePath, path.basename(bundlePath)) :
			`${baseURI.replace(/\/$/, "")}/${bundlePath}`;
