...
function generateConfig({ extensions = [], // eslint-disable-next-line indent
	...
	if(compact) {
		cfg.compact = true;
		plugins = plugins.concat(chooseCompaction(compact));
	}
	cfg.plugins = plugins;
...
function chooseCompaction(type) {
	if(type === "compact") {
		return require("rollup-plugin-cleanup")();
	} else if(type === "minify") {
		let { terser } = require("rollup-plugin-terser");
		return terser({ compress: false, mangle: false });
	} else if(type === "mangle") {
		let { terser } = require("rollup-plugin-terser");
		return terser({ compress: false, mangle: true });
	} else {
		abort(`unknown compaction option ${type}`);
	}
}
