...
// * `sourcemap`, if truthy, activates inline source-map generation
...
function generateConfig({ extensions = [], // eslint-disable-next-line indent
		externals, format, moduleName, // eslint-disable-next-line indent
		exports, esnext, jsx, typescript, // eslint-disable-next-line indent
		sourcemap, compact }, { browsers, resolvePath }) {
	let cfg = { sourcemap };
	...
	exports = determineExports(exports, cfg.format, moduleName);
	if(exports) {
		cfg.name = exports;
	}
...
function determineModuleFormat(format = "iife") {
	...
	switch(_format) {
	case undefined:
		abort(`unrecognized module format: ${repr(format)}`);
		return;
	case false:
...
function determineExports(exports, format, moduleName) {
	if(moduleName) {
		console.error(`WARNING: ${repr("moduleName", false)} is deprecated in ` +
				`favor of ${repr("exports", false)}`);
		exports = moduleName;
	}

	if(exports && NAMELESS_MODULES.has(format)) {
