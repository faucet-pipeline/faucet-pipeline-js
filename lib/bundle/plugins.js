...
exports.determinePlugins = function({ esnext, typescript, jsx, sourcemaps, browsers, compact }) {
	...
	if(esnext || typescript || jsx || compact === "minify" || compact === "mangle") {
		let { default: swc } = loadExtension("rollup-plugin-swc",
				loadExtensionError(esnext, typescript, jsx, compact));
...
function loadExtensionError(esnext, typescript, jsx, jsmin) {
	let extensions = [];
	if(esnext) {
		extensions.push("faucet-pipeline-esnext");
	}
	if(typescript) {
		extensions.push("faucet-pipeline-typescript");
	}
	if(jsx) {
		extensions.push("faucet-pipeline-jsx");
	}
	if(jsmin) {
		extensions.push("faucet-pipeline-jsmin");
	}
	return `failed to activate ${extensions.join(", ")}`;
}
